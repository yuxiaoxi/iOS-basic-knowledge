...
将函数和其执行的上下文封装起来的对象，本质是个对象。

#### block 变量截获

1. 对值类型的局部变量截获是值截获
2. 对局部静态变量截获是指针截获
3. 对全局变量和全局静态变量不截获，直接取值
4. 对对象类型的局部变量截获是连同所有权修饰符一起截获

#### block 的几种形式

1. 全局 block(gloab block): 是指未对外部变量进行引用的 block
2. 栈 block: 对外部变量引用，但未进行 copy 的 block
3. 堆 block: 栈 block 进行 copy 后，会复制到堆上

注意：栈 block 被 copy 后会变成堆 block；堆 block 被 copy 后引用计算会加 1；全局 block 被 copy 没有任何操作。

#### __ block 修饰符 

__ block 修饰符：对截获变量进行赋值操作需要添加此修饰符

使用场景：需要对局部变量进行赋值操作时，包括：基本数据类型（值类型）和对象类型的局部变量
